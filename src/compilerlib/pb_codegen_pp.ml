  ...
  F.scope sc (fun sc ->
      ...
      List.iter
        (fun { Ot.vc_constructor; vc_field_type; _ } ->
          ...
          | Ot.Vct_nullary ->
            F.line sc
            @@ sp "| %s_types.%s  -> Format.fprintf fmt \"%s\"" module_prefix
                 vc_constructor vc_constructor
          | Ot.Vct_non_nullary_constructor field_type ->
            let field_string_of = gen_field field_type in
            F.line sc
            @@ sp
                 "| %s_types.%s x -> Format.fprintf fmt \"@[<hv2>%s(@,\
                  %%a)@]\" %s x" module_prefix vc_constructor vc_constructor
                 field_string_of)
        v_constructors)
...
let gen_sig ?and_ t sc =
  ...
  let f type_name =
    F.line sc
    @@ sp "val pp_%s : Format.formatter -> %s_types.%s -> unit " type_name
